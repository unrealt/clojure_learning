...
1. 使用模型编辑工具开发[模型定义元数据](#3-模型定义)。
2. 使用模型编辑工具开发[模型持久化定义元数据](#5-模型持久化定义)。（可选）
3. 在代码中使用[模型定义](#31-模型定义只读接口)和[模型对象](#4-模型对象)。

在初期阶段也可以使用工作副本接口，通过代码来定义模型和持久化定义，但不推荐。

在实施期，主要关心模型是怎样在实际数据库中进行存储，需要进行以下操作：

1. 使用实施工具，根据模型的定义和持久化定义，生成针对具体数据库的[存储模型定义元数据](#6-存储模型)。
